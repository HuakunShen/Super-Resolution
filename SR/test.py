...
def main_no_target(in_path: pathlib.Path, output_dir: pathlib.Path, weight_path: pathlib.Path):
...
def main_with_target(in_path, out_dir, label_path, weight_path):
    model = torch.load(weight_path)
    image_names = sorted(os.listdir(in_path))
    low_res_psnr_sum = 0
    computed_psnr_sum = 0
    for image_name in tqdm(image_names):
        img = Image.open(in_path / image_name)
        computed_img = compute(model, img)
        label_img = Image.open(label_path / image_name)
        fig, axes = plt.subplots(nrows=1, ncols=3, figsize=(30, 10))
        low_res_psnr = psnr_PIL(img, label_img)
        ...
        axes[2].set_title(
            f"High Res Image", fontsize=20)
        for i in range(3):
            axes[i].axis('off')
        output_path = out_dir / (image_name.split('.')[0] + ".png")
        fig.savefig(output_path)
        plt.close()
        img.close()
        label_img.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument("-w", "--weight", help="Weight File Path", required=True)
    args = parser.parse_args()
    in_path = pathlib.Path(args.input).absolute()
    out_path = pathlib.Path(args.output).absolute()
    label_path = pathlib.Path(args.label).absolute() if args.label is not None else None
    ...
    if not out_path.exists():
        ...
        logger.warning(f"output path {out_path} exists, your output directory may be removed")
    ...
    if os.path.isfile(in_path):
        ...
        if label_path is None:
            ...
            main_with_target(in_path, out_path, label_path, weight_path)
